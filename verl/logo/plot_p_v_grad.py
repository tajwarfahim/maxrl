...
from losses import cross_entropy, reinforce_loss, reinforce_with_baseline_loss, grpo_loss, p_normalization
import random
...
def train_one_epoch(model, optimizer, scheduler, train_loader, val_loader, device, epoch, loss_fn, cfg: Config, global_step=0):
    ...
    for step, (images, targets) in tqdm(enumerate(train_loader), 
        ...
        if (global_step % cfg.eval_interval) == 0:
            ...
            compute_p_vs_grad(model, val_loader, device, epoch, loss_fn, optimizer, global_step, cfg)
...
def compute_p_vs_grad(model, val_loader, device, epoch, loss_fn, optimizer, global_step, cfg):
    model.eval()
    ...
    for images, targets in tqdm(val_loader, desc="Validation", total=len(val_loader)):
        ...
        for i in range(images.shape[0]):
            ...
            loss = loss_fn(logits, torch.Tensor([target]).long().to(logits.device))
            loss.backward()
